...
# Page configuration
st.set_page_config(page_title="ระบบจัดการร้านรถกระบะ",page_icon="🛻",layout="wide")
...
def show_dialogAddProd():
    ...
    if st.button("เพิ่มสินค้า",type="primary",key="add_product"):
        if name and price > 0 and image_url:
            db.save_product(id,name, price, brand, image_url)
            st.rerun()
        else:
...
def show_dialogEditProd(row_idx,product):
    ...
    if product:
        with st.form(key=f"edit_form_{product['id']}"):
            ...
            edit_brand = st.selectbox("หมวดหมู่", ["อาหารจานหลัก", "ของทานเล่น", "เครื่องดื่ม", "ของหวาน"], index=["อาหารจานหลัก", "ของทานเล่น", "เครื่องดื่ม", "ของหวาน"].index(product["brand"]))
            ...
            if st.form_submit_button("บันทึก"):
                db.update_product(row_idx,edit_id, edit_name, edit_price, edit_brand, edit_status, edit_image_url)
    ...
    if products:
        ...
        for idx, product in enumerate(products): # Iterate over a copy
            with cols[idx % 4]:
                ...
                st.write(f"หมวดหมู่: {product['brand']}")
...
def order_page():
    ...
    with col2:
        ...
        if hasattr(st.session_state, 'order_items') and st.session_state.order_items:
            ...
            custumer_name = st.text_input("ชื่อลูกค้า")
            special_instructions = st.text_area("บันทึกเพิ่มเติม")
           
           
            if st.button("ยืนยันการสั่งซื้อ"):
                if custumer_name:
                    db.save_order(custumer_name, order_items_list, total, special_instructions)  # Pass the list
...
def order_management_page():
    ...
    if orders:
        for idx, order in filtered_orders_df.iterrows():
            ...
            with st.expander(f"ออเดอร์ {idx+1} - ลูกค้า:{order['custumer_name']}:ยอดรวม: ฿{order['total']:.2f} ({order_date})"):
