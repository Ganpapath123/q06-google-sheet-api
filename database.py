...
# Sheet configuration
SHEET_ID = "1XdWwHHFeZimQCosZoj_hHgbv_HqL7Q_nvBqNvCvY4kA"  # Replace with your Google Sheet ID
...
def load_products():
    products_sheet = get_worksheet('pickuptrucklist')
    if products_sheet:
        products = products_sheet.get_all_records()
        return products if products else []
    return []

def save_product(id,name, price, brand, image_url, status='active'):
    products_sheet = get_worksheet('pickuptrucklist')
    if products_sheet:
        products_sheet.append_row([id,name, float(price), brand, status, image_url])
        st.success(f"บันทึกสินค้า '{name}' เรียบร้อยแล้ว")

def update_product(row_idx, id, name, price, brand, status, image_url):
    products_sheet =get_worksheet('pickuptrucklist')
    if products_sheet:
        try:
            products_sheet.update(f'A{row_idx+2}:F{row_idx+2}', [[id, name, float(price), brand, status, image_url]])
...
def load_orders():
    orders_sheet = get_worksheet('pickuptruckorder')
    if orders_sheet:
